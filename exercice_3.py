...
def main():
    ...
    user = User("bob", "mdp")
    moderateur = Moderator("maude", "pass")

    thread = User.make_thread(self=  user, title="Premmier message", content="Bonjour")
    thread.display()

    moderateur.post(thread, content="première réponse")
    thread.display()

    hors_sujet = user.post(thread, content="chocolat")
    reponse = moderateur.post(thread, content="HS")
    thread.display()


    moderateur.delete(thread, hors_sujet)
    moderateur.delete(thread=thread, post=reponse)

    image = PNGImageFile("chocolat",size=2)
    user.post(thread, content="tablette de chocolat", file=image)
    moderateur.post(thread=thread, content="wow")
    thread.display()
